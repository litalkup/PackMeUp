...
(function (global) {
  ...
  var KEYWORDS = {
    ...
    electronics: [
      ...
      'מכשיר קשר', 'שעון', 'שעון חכם', 'מטען לרכב', 'סטיק', 'פיט ביט', 'fitbit', 'שעון כושר', 'מכשיר'
    ],
    toiletries: [
      ...
      'מטהר ידיים', 'אטמי אוזניים', 'קרם אחרי שיזוף', 'צמר גפן', 'נססר', 'כלי רחצה', 'אולדייז', 'ליפסטיק', 'דיאו', 'עדשות',
      'עדשות חד פעמיות', 'עדשות חדפ', 'אלוורה', 'מני פדי', 'לק', 'מסכת פנים'
    ],
    health: [
      ...
      'ספריי נגד יתושים', 'דוחה יתושים', 'אפטר בייט', 'חוסם עורקים', 'קונדומים', 'וולטרן', 'טיפות', 'טיפות אף', 'טיפות עיניים', 'משחה לכאבים',
      'קרם הרגעה'
    ],
    military: [
    ...
    gear: [
      ...
      'שקיות', 'שקית', 'מנעול', 'מנעולים', 'את חפירה קטן', 'כלי עבודה', 'בקבוק', 'דאפל בג', 'דאפל'
    ],
    sleep: [
    ...
    clothing: [
      ...
      'כביסה', 'פוך', 'שכבה ראשונה', 'קרדיגן', 'סריג', 'טייץ', 'טייצים', 'גופיה', 'חזיה',
      'חולצת פסים', 'תכשיטים', 'שרשרת', 'עגילים', 'טבעת', 'משקפיים', 'משקפי ראייה'
    ],
    food: [
   ...
   */
  /*
   * Pass { prefixes: false } for the readings that leave prefixes alone.
   * Peeling both of the words being compared lets unrelated ones meet on a
   * shared tail - שלישי and שישי both reach ישי - so a comparison peels one
   * side at a time.
   */
  function forms(word, options) {
    var out = [];
    var peel = !options || options.prefixes !== false;
    function add(value) {
      if (value && out.indexOf(value) === -1) out.push(value);
    }
    (peel ? prefixForms(word) : [word]).forEach(function (reading) {
      add(reading);
      add(stem(reading));
